...
describe('boot.ts', () => {
  ...
  describe('readInjectedConfig', () => {
    it('returns the injected config when __SOLANDER__ is set', async () => {
      ...
      (globalThis as any).__SOLANDER__ = { serverUrl: 'https://chat.example.com', desktop: true };

      const config = readInjectedConfig();
      expect(config).toEqual({ serverUrl: 'https://chat.example.com', desktop: true });
    });
  ...
  describe('setServerConfig', () => {
    it('calls invoke with the server URL', async () => {
      ...
      expect(invoke).toHaveBeenCalledWith('set_server_url', { url: 'https://chat.example.com' });
    });
  });
