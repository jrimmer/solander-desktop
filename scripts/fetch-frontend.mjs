...
if (!existsSync(BUILD_CACHE)) {
	...
	runOrExit(
		`git clone --depth 1 --branch ${tag} ${UPSTREAM_REPO} ${BUILD_CACHE}`,
		{
			cwd: ROOT,
			stdio: "inherit",
		},
	...
	runOrExit(`git fetch --depth 1 origin tag ${tag}`, {
		cwd: BUILD_CACHE,
		stdio: "inherit",
	});
...
runOrExit("pnpm install --no-frozen-lockfile", {
	cwd: BUILD_CACHE,
	stdio: "inherit",
});

console.log("[fetch-frontend] Building upstream frontend...");
runOrExit("pnpm run build:frontend", {
	cwd: BUILD_CACHE,
	stdio: "inherit",
});
